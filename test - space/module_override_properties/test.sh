...
#shrendd.module.$_the_module.properties
export SHRENDD_MODULE_OTHER_PROPERTIES="other/properties/properties.yml"
...
export test_results="override module properties location, and run unwind tests:\n"
...
if [ "$_check" == "not found" ]; then
  export test_results="$test_results\t${_TEST_ERROR}other: had no errors rendering, and that was unexpected. failed${_CLEAR_TEXT_COLOR}\n"
else
  export test_results="$test_results\tother: had errors rendering, and that was expected. passed\n"
fi
